...
function VAE.get_encoder(channels, naf, z_dim)
    ...
    encoder:add(nn.SpatialConvolution(naf * 16, naf * 32, 4, 4, 2, 2, 1, 1))
    encoder:add(nn.SpatialBatchNormalization(naf * 32)):add(nn.ReLU())

    zLayer = nn.ConcatTable()
    zLayer:add(nn.SpatialConvolution(naf * 32, z_dim, 4, 4))
    zLayer:add(nn.SpatialConvolution(naf * 32, z_dim, 4, 4))
    encoder:add(zLayer)
...
function VAE.get_decoder(channels, ngf, z_dim)
    ...
    decoder:add(nn.SpatialFullConvolution(z_dim, ngf * 32, 4, 4))
    decoder:add(nn.SpatialBatchNormalization(ngf * 32)):add(nn.ReLU(true))
    decoder:add(nn.SpatialFullConvolution(ngf * 32, ngf * 16, 4, 4, 2, 2, 1, 1))
